    ...
    return (
        <div className="p-6">
            <div className="relative aspect-video">
                <Image
                    src={
                        (course?.thumbnail?.key && process.env.S3_BUCKET_NAME) ? `${process.env.S3_ENDPOINT?.replace(/\/$/, '') || `https://${process.env.S3_BUCKET_NAME}.s3.amazonaws.com`}/${course.thumbnail.key}` : (course?.attachments?.[0]?.url ?? '')
                    }
